...
#include "../Headers/PathE.hpp"

using namespace PathE2D;

PathE::PathE() {};

PathE::PathE(PathE path1, PathE path2){
    ...
    PathECoordinates path_coordinates = PathECoordinates(start_point, end_point, maxCurvature);
    setFinder<DubinPathEFinder>(path_coordinates);
    
}

PathE::PathE(Position start_point, Position end_point, double curvature, Map *map_i) {
...
template <class T>
PathE::PathE(Position start_point, Position end_point, double curvature, Map *map_i, T* pathFinder, bool complex){
    setStartPoint(start_point);
    setEndPoint(end_point);
    setMaxCurvature(curvature);
    setMap(map_i);
...
template <class T>
PathE::PathE(PathECoordinates coordinates, Map *map, T* pathFinder, bool complex){
...
void PathE::setEndPoint(Position end_point) {
    this->end_point = end_point;
}

Position PathE::getEndPoint() {
    return end_point;
}

Position PathE::getStartPoint() {
    return start_point;
}

void PathE::setMaxCurvature(double curvature) {
    maxCurvature = curvature;
}

double PathE::getMaxCurvature() {
    return maxCurvature;
}

void PathE::setStartPoint(Position start_point) {
    this->start_point = start_point;
}

void PathE::setLines(std::vector<Line> lines) {
...
std::vector<Line> PathE::getLines() {
    return lines;
}


void PathE::setLength(double length){
    this->length = length;
};
double PathE::getLength(){
    return length;
};
...
void PathE::findPathE() {
    ...
    do{
        ...
        dubin_lines = finder->shortestPathE(alt_points);
...
void PathE::split(PathE &path, cv::Point intermediate){
    ...
    while(orientation_i < orientation_estimation + 2.01*M_PI){
        ...
        //one = PathE(PathECoordinates(path.start_point, point, path.maxCurvature),path.map,false);
        
        two = PathE(PathECoordinates(point, path.end_point, path.maxCurvature),path.map, new T(PathECoordinates(point, path.end_point, path.maxCurvature),path.map),false);
...
template <class T>
void PathE::setFinder(PathECoordinates pc){

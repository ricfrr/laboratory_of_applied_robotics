...
Path2D::PathCoordinates Robot::initialize(){
    // initial position
    int in_x = 0;
    int in_y = 1;
    ...
    Path2D::Position in_position = Path2D::Position(in_coordinates,in_orient);
    // final position
    int f_x = 4;
    int f_y = 3;
    double f_orient = M_PI/3.0;
    cv::Point2d f_coordinates = Point2d(f_x,f_y);
    Path2D::Position f_position = Path2D::Position(f_coordinates,f_orient);
    
    
    Path2D::PathCoordinates car = Path2D::PathCoordinates(in_position,f_position,Kmax);
    return car;
}
